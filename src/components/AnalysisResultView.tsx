    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center space-x-2 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">
              ...
              <span>•</span>
              <span className="text-emerald-400">{result.timeframe || 'Intraday'}</span>
              <span>•</span>
              <span>{new Date(result.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
            </div>

            <div className="flex items-center space-x-3">
              <div className={`p-3 rounded-2xl border ${signalInfo.bg} shadow-lg`}>
                {signalInfo.icon}
              </div>
              <div>
          ...
          {/* Confidence Score & R:R Summary */}
          <div className="flex flex-col sm:flex-row lg:flex-col items-start sm:items-center lg:items-end gap-3 bg-slate-950/70 p-4 rounded-xl border border-slate-800">
            <div>
              <div className="flex items-center justify-between text-xs mb-1">
                <span className="text-slate-400 font-medium">{t.confidenceScore}:</span>
                <span className="font-extrabold text-emerald-400 ml-2">{result.confidenceScore}%</span>
              </div>
            ...
            <div className="text-left sm:text-right">
              <span className="text-[10px] text-slate-400 uppercase tracking-wider block">{t.riskRewardRatio}</span>
              <span className="text-lg font-black text-white">{result.overallRiskRewardRatio || '1 : 2.5'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-5 pt-4 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={onOpenChat}
              className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-semibold transition flex items-center space-x-1.5 cursor-pointer"
            >
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-gradient-to-r from-teal-500/20 to-cyan-500/20 hover:from-teal-500/30 hover:to-cyan-500/30 text-cyan-300 border border-cyan-500/40 text-xs font-bold transition flex items-center space-x-1.5 cursor-pointer shadow-sm"
            >
              ...
              {result.economicCalendarWarning.upcomingNewsEvents.map((ev, i) => (
                <div key={i} className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-xs space-y-1">
                  <div className="flex items-center justify-between font-bold text-slate-200">
                    ...
                    <span className="text-[10px] text-amber-400">{ev.date}</span>
                  </div>
                  <p className="text-[11px] text-slate-400">{ev.warningText}</p>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {result.methodologyConfluences.map((conf, idx) => (
              <div key={idx} className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-200">{conf.methodology}</span>
                  <span
                ...
                <p className="text-xs text-slate-300 leading-relaxed">{conf.keyObservation}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl flex flex-col justify-between">
          <h3 className="text-sm font-bold text-slate-100 flex items-center justify-between mb-4 pb-2 border-b border-slate-800">
          ...
          <div className="space-y-3.5">
            {/* Entry Level */}
            <div className="p-3 bg-slate-950 border border-blue-500/30 rounded-xl relative overflow-hidden">
            ...
            {/* Stop Loss Level */}
            <div className="p-3 bg-slate-950 border border-red-500/30 rounded-xl relative overflow-hidden">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-red-400 font-mono">-{result.stopLoss?.distancePercent ?? 0}%</span>
              </div>
            ...
            <div className="space-y-2">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{t.takeProfit1} / {t.takeProfit2} / {t.takeProfit3}</div>
              {(result.takeProfitTargets || []).map((tp) => (
                <div
                  key={tp.target}
                  className="p-2.5 bg-slate-950 border border-emerald-500/20 rounded-xl flex items-center justify-between"
                >
                  <div>
                    ...
                    <div className="text-[10px] text-slate-400 mt-0.5">{tp.description}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs font-black text-emerald-400">R:R 1:{tp.riskRewardRatio}</div>
                    <div className="text-[9px] text-slate-500">{tp.closePercentage}%</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-4 pt-3 border-t border-slate-800 text-xs space-y-1 text-slate-400">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-slate-200">{result.riskManagement?.suggestedPositionSizePercent ?? 1}%</span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-800">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-sm font-bold text-slate-100">{t.keyLevels}</h3>
            </div>

            <div className="flex items-center space-x-2">
              <button
                onClick={() => setShowChartOverlay(!showChartOverlay)}
                className="px-2.5 py-1 rounded-lg bg-slate-800 text-slate-300 hover:text-white text-xs font-medium flex items-center space-x-1 transition cursor-pointer"
              >
            ...
            {currentImage ? (
              <div className="relative w-full h-full">
                ...
                {showChartOverlay && (() => {
                  ...
                  return (
                    <div className="absolute inset-0 pointer-events-none p-2 sm:p-4 bg-slate-950/20 overflow-hidden">
                      ...
                      >
                        <span className="absolute top-1 left-2 text-[9px] font-extrabold text-red-300 uppercase tracking-widest bg-red-950/80 px-1.5 py-0.5 rounded border border-red-500/30">
                          {t.stopLoss} Zone
                        </span>
                      </div>
                      ...
                      >
                        <span className="absolute bottom-1 left-2 text-[9px] font-extrabold text-emerald-300 uppercase tracking-widest bg-emerald-950/80 px-1.5 py-0.5 rounded border border-emerald-500/30">
                      ...
                      >
                        <div className="bg-red-950/90 border border-red-500/80 text-red-200 text-[10px] font-extrabold px-2 py-0.5 rounded shadow-lg flex items-center space-x-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />
                          <span>STOP LOSS: {overlay.sl.priceStr}</span>
                        </div>
                        <span className="bg-red-950/90 border border-red-500/80 text-red-300 text-[9px] font-bold px-1.5 py-0.5 rounded shadow">
                          SL
                        </span>
                      </div>
                      ...
                      >
                        <div className="bg-cyan-950/90 border border-cyan-400 text-cyan-200 text-[10px] font-extrabold px-2 py-0.5 rounded shadow-lg flex items-center space-x-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-cyan-400" />
                          <span>ENTRY: {overlay.entry.priceStr}</span>
                        </div>
                        <span className="bg-cyan-950/90 border border-cyan-400 text-cyan-200 text-[9px] font-bold px-1.5 py-0.5 rounded shadow">
                          ENTRY
                        </span>
                      </div>
                      ...
                      {overlay.tps.map((tp) => (
                        <div
                          ...
                          style={{ top: `${tp.top}%` }}
                        >
                          <div className="bg-emerald-950/90 border border-emerald-500/80 text-emerald-200 text-[10px] font-extrabold px-2 py-0.5 rounded shadow-lg flex items-center space-x-1">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                            <span>
                              TP{tp.target}: {tp.rawPrice} {tp.closePercent ? `(${tp.closePercent}%)` : ''}
                            </span>
                          </div>
                          <span className="bg-emerald-950/90 border border-emerald-500/80 text-emerald-300 text-[9px] font-bold px-1.5 py-0.5 rounded shadow">
                            TP{tp.target}
                          </span>
                        </div>
                      ))}
              ...
              <div className="text-slate-500 text-xs">Chart image not available</div>
            )}
          </div>
              ...
              {result.uploadedImages.map((img, i) => (
                <button
                  ...
                  className={`relative w-16 h-10 rounded-lg overflow-hidden border transition cursor-pointer ${
                    selectedImageIdx === i ? 'border-emerald-400 ring-2 ring-emerald-400/30' : 'border-slate-800 opacity-60'
                  }`}
                >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        ...
        <div className="flex border-b border-slate-800 overflow-x-auto bg-slate-950/50">
          <button
            onClick={() => setActiveTab('levels')}
            className={`px-5 py-3 text-xs font-bold transition flex items-center space-x-2 border-b-2 whitespace-nowrap cursor-pointer ${
              activeTab === 'levels'
        ...
        <div className="p-5 sm:p-6">
          ...
          {activeTab === 'levels' && (
            <div className="space-y-4">
              <h4 className="text-sm font-bold text-slate-200">{t.priceActionStructures}</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {(result.priceActionStructures || []).map((pas, i) => (
                  <div key={i} className="p-3.5 bg-slate-950 border border-slate-800 rounded-xl space-y-1">
                    ...
                    <p className="text-xs text-slate-300 leading-relaxed">{pas.description}</p>
                  </div>
                ))}
              </div>
                ...
                <div className="p-3 bg-emerald-950/20 border border-emerald-500/20 rounded-xl">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {(result.keyLevels?.support || []).map((lvl, idx) => (
                      <span key={idx} className="px-2 py-0.5 bg-emerald-500/20 text-emerald-300 font-mono text-xs rounded font-bold">
                        {lvl}
                      </span>
                    ))}
                  </div>
                ...
                <div className="p-3 bg-red-950/20 border border-red-500/20 rounded-xl">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {(result.keyLevels?.resistance || []).map((lvl, idx) => (
                      <span key={idx} className="px-2 py-0.5 bg-red-500/20 text-red-300 font-mono text-xs rounded font-bold">
                        {lvl}
                      </span>
                    ))}
                  </div>
          ...
          {activeTab === 'candles' && (
            <div className="space-y-3">
              <h4 className="text-sm font-bold text-slate-200 mb-3">{t.candlestickPatterns}</h4>
              {(result.candlestickPatterns || []).map((cp, idx) => (
                <div
                  key={idx}
                  className="p-3.5 bg-slate-950 border border-slate-800 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-2"
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-400">{cp.significance}</p>
                  </div>

                  <div className="text-[11px] text-slate-500 font-mono bg-slate-900 px-2.5 py-1 rounded border border-slate-800 self-start sm:self-auto">
                    {cp.location}
                  </div>
                </div>
              ))}
          ...
          {activeTab === 'mentor' && (
            <div className="space-y-4">
              <div className="flex items-center space-x-3 p-3 bg-emerald-950/30 border border-emerald-500/30 rounded-xl text-emerald-300 text-xs">
              ...
              <div className="text-sm text-slate-200 leading-relaxed whitespace-pre-line bg-slate-950 p-4 rounded-xl border border-slate-800 font-sans">
                {result.mentorAdvice}
              </div>
            </div>
          )}
          ...
          {activeTab === 'checklist' && (
            <div className="space-y-2.5">
              <h4 className="text-sm font-bold text-slate-200 mb-3">{t.tradeChecklist}</h4>
              {(result.tradeChecklist || []).map((item, idx) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className="text-xs font-bold">{item.rule}</div>
                      <div className="text-[11px] text-slate-400 mt-0.5">{item.comment}</div>
                    </div>
                  </div>
